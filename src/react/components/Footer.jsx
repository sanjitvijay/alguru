...
function Footer() {
    ...
    const {question, setQuestion, askQuestion, isChat, isChatAvailable, isVoiceAvailable, chatUnlockTime, voiceUnlockTime, isValidPage} = useAppContext();
    ...
    return (
        ...
        {isValidPage() &&
            (isChat ?
                ...
                : (<div className='prose text-md'>Continue using chat from {format(new Date(chatUnlockTime),'EEEE, h:mma')}</div>)
                ) :
                (!isVoiceAvailable && (<div className='prose text-md'>Continue using voice assistant from {format(new Date(voiceUnlockTime),'EEEE, h:mma')}</div>))
            )
        }
        </div>
    )
