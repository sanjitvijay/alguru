...
export const AppContextProvider = ({ children }) => {
  ...
  const instructions = `You are a coding tutor who is an expert at leetcode 
    ...
    chrome.scripting.executeScript(
      {
        ...
        function: function () {
          let selectors = {
            title:
              "div > div.flex.w-full.flex-1.flex-col.gap-4.overflow-y-auto.px-4.py-5 > div.flex.items-start.justify-between.gap-4 > div.flex.items-start.gap-2 > div > a",
            description:
              "div > div.flex.w-full.flex-1.flex-col.gap-4.overflow-y-auto.px-4.py-5 > div.elfjS",
            ...
            language:
              "div.flex.h-8.items-center.justify-between.border-b.p-1.border-border-quaternary.dark\\:border-border-quaternary > div.flex.flex-nowrap.items-center > div:nth-child(1)",
          };
      ...
      async (results) => {
        ...
        if (storedProblem.problem !== results[0].result.title) {
          ...
          const savedMessages = await chrome.storage.session.get([
            "chatHistory",
            "messageHistory",
          ]);
          savedMessages.chatHistory
            ? setChatHistory(savedMessages.chatHistory)
            : setChatHistory([]);
          savedMessages.messageHistory
            ? setMessageHistory(savedMessages.messageHistory)
            : setMessageHistory(initialMessageHistory);
        }
      },
    );
  ...
  const askQuestion = async () => {
    ...
    for await (const chunk of stream) {
      responseRef.current =
        responseRef.current + (chunk.choices[0]?.delta?.content || "");
  ...
  const setUsageCounts = async () => {
    ...
    result.chatExpired &&
      setChatUnlockTime(result.chatExpired + cooldownHours * 60 * 60 * 1000);
    result.voiceExpired &&
      setVoiceUnlockTime(result.voiceExpired + cooldownHours * 60 * 60 * 1000);

    const chatElapsedTime =
      (new Date().getTime() - result.chatExpired) / (1000 * 60 * 60);
    const voiceElapsedTime =
      (new Date().getTime() - result.voiceExpired) / (1000 * 60 * 60);
